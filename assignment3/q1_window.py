        ...
        for i, word in enumerate(sentence):
            ...
            for j in range(i-window_size, i+window_size + 1):
                if j < 0:
                    ...
                    words.extend(sentence[j])
            windowed_data.append((words, labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor = tf.Variable(initial_value=self.pretrained_embeddings)
        embeddings = tf.reshape(tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder),
    ...
    def add_prediction_op(self):
        ...
        normal_initializer = tf.contrib.layers.xavier_initializer()
        const_initializer = tf.constant_initializer()

        w = tf.get_variable("W1", (self.config.n_window_features * self.config.embed_size, self.config.hidden_size), initializer=normal_initializer)

        b1 = tf.get_variable("b1", (self.config.hidden_size,), initializer=const_initializer)
        h = tf.nn.relu(tf.matmul(x, w) + b1)

        h_drop = tf.nn.dropout(h, dropout_rate)


        u = tf.get_variable("u", (self.config.hidden_size , self.config.n_classes ), initializer=normal_initializer)

        b2 = tf.get_variable("b2",  (self.config.n_classes,), initializer=const_initializer)
