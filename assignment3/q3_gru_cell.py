...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            const = tf.constant_initializer(0)

            U_z = tf.get_variable("U_z", (self.input_size, self.state_size), initializer=init)
            W_z = tf.get_variable("W_z", (self.state_size, self.state_size), initializer=init)
            b_z = tf.get_variable("b_z", (self.state_size), initializer=const)
            U_r = tf.get_variable("U_z", (self.input_size, self.state_size), initializer=init)
            W_r = tf.get_variable("W_r", (self.state_size, self.state_size), initializer=init)
            b_r = tf.get_variable("b_r", (self.state_size), initializer=const)
            U_o = tf.get_variable("U_o", (self.input_size, self.state_size), initializer=init)
            W_o = tf.get_variable("W_o", (self.state_size, self.state_size), initializer=init)
            b_o = tf.get_variable("b_r", (self.state_size), initializer=const)
