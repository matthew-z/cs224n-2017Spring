...
def pad_sequences(data, max_length):
    ...
    0, 0], and max_length = 5, we would construct
        ...
        - a new label sequence: [1, 0, 0, 4, 4], and
    ...
    Args:
        data: is a list of (sentence, labels) tuples. @sentence is a list
            ...
            Manning pad_sequencesis amazing" and labels "PER PER O O" would become
    ...
    for sentence, labels in data:
        ...
        n = len(sentence)
        sentence_ = (sentence + [zero_vector[:] for _ in range(max(0, max_length - n))])[:max_length]
        labels_ = (labels + [zero_label for _ in range(max(0, max_length - n))])[:max_length]
        mask_ = ([True for _ in range(n)] + [False for _ in range(max(0, max_length - n))])[:max_length]
        ret.append([sentence_, labels_, mask_])
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable("b2", shape=(self.config.n_classes), initializer=const)
        h = tf.zeros([tf.shape(x)[0], self.config.hidden_size*2])
        ### END YOUR CODE

        cell = tf.contrib.rnn.LSTMCell(Config.hidden_size, state_is_tuple=False)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_dropout = tf.nn.dropout(o, self.dropout_placeholder)
                preds.append(tf.matmul(o_dropout, U) + b2)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        # mask_label = tf.boolean_mask(tensor=self.labels_placeholder, mask=self.mask_placeholder)
        # mask_pred = tf.boolean_mask(tensor=preds, mask=self.mask_placeholder)

        loss_unmask = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)

        loss = tf.reduce_mean(tf.boolean_mask(tensor=loss_unmask, mask=self.mask_placeholder))
        ### END YOUR CODE
        return loss
