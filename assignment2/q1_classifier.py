...
class SoftmaxModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        ### YOUR CODE HERE
        ### END YOUR CODE
        return {self.input_placeholder:inputs_batch, self.labels_placeholder:labels_batch}
    ...
    def add_prediction_op(self):
        ...
        w = tf.Variable(tf.zeros(w_shape), tf.float32)

        b_shape = [self.config.n_classes]
        b = tf.Variable(tf.truncated_normal(b_shape, stddev=0.01))

        ### END YOUR CODE
        return softmax(tf.matmul(self.input_placeholder, w) + b)
